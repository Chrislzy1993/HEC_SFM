...
public:
  ...
  CenterChooser(const Distance& distance, const std::vector<ElementType*>& points)
    : distance_(distance), points_(points){};
    ...
    for (centerCount = 1; centerCount < k; centerCount++)
    {
      ...
      for (index = 0; index < n; index++)
      {
        ...
        if (closestDistSq[index] > kSpeedUpFactor * (float)furthest)
        {
          ...
          for (int i = 0; i < n; i++)
          {
            newPot += std::min(distance_(points_[indices[i]], points_[indices[index]], cols_), closestDistSq[i]);
          }
      ...
      {
        closestDistSq[i] =
            std::min(distance_(points_[indices[i]], points_[indices[bestNewIndex]], cols_), closestDistSq[i]);
      }
    }
