...
class SaveArchive : public OutputArchive<SaveArchive>
{
  ...
  void flushBlock()
  {
    ...
    if (first_block_)
    {
      ...
      compSz = LZ4_compress_HC_continue(lz4Stream, buffer_ + headSz, compressed_buffer_ + headSz, offset_ - headSz,
    ...
    else
    {
      ...
      // Do the compression for the block
      compSz = LZ4_compress_HC_continue(lz4Stream, buffer_, compressed_buffer_ + headSz, offset_,
                                        LZ4_COMPRESSBOUND(BLOCK_BYTES));
...
class LoadArchive : public InputArchive<LoadArchive>
{
  ...
  void decompressAndLoadV10(FILE* stream)
  {
    ...
    // Extract body
    size_t usedSz = LZ4_decompress_safe(compBuffer + headSz, buffer_ + headSz, compressedSz, uncompressedSz);
  ...
  void loadBlock(char* buffer_, size_t compSz, FILE* stream)
  {
    ...
    const int decBytes =
        LZ4_decompress_safe_continue(lz4StreamDecode, compressed_buffer_, buffer_, compSz, BLOCK_BYTES);
    if (decBytes <= 0)
    {
