...
{
  TestSearch<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_, flann::SearchParams(-1),
                       0.9, gt_indices, gt_dists);
}

TEST_F(LshIndex_Brief100K, TestSearch2)
{
  TestSearch2<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_, flann::SearchParams(-1),
                        0.9, gt_indices, gt_dists);
}

TEST_F(LshIndex_Brief100K, TestAddIncremental)
{
  TestAddIncremental<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_,
                               flann::SearchParams(-1), 0.9, gt_indices, gt_dists);
}

TEST_F(LshIndex_Brief100K, TestIncremental2)
{
  TestAddIncremental2<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_,
                                flann::SearchParams(-1), 0.9, gt_indices, gt_dists);
}

TEST_F(LshIndex_Brief100K, TestRemove)
{
  TestRemove<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_, flann::SearchParams(-1));
}

TEST_F(LshIndex_Brief100K, TestSave)
{
  TestSave<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_, flann::SearchParams(-1), 0.9,
                     gt_indices, gt_dists);
}

TEST_F(LshIndex_Brief100K, TestCopy)
{
  TestCopy<Distance>(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_, flann::SearchParams(-1), 0.9,
                     gt_indices, gt_dists);
}

TEST_F(LshIndex_Brief100K, TestCopy2)
{
  TestCopy2<flann::LshIndex<Distance> >(data, flann::LshIndexParams(12, 20, 2), query, indices, dists, k_nn_,
                                        flann::SearchParams(-1), 0.9, gt_indices, gt_dists);
}
