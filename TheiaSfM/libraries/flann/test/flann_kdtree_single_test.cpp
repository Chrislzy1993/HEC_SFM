...
{
  TestSearch<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                                flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestSearch2)
{
  TestSearch2<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                                 flann::SearchParams(-1), 0.99, gt_indices);
}
...
TEST_F(KDTreeSingle, TestSearchPadded)
{
  ...
  TestSearch<L2_Simple<float> >(data2, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                                flann::SearchParams(-1), 0.99, gt_indices);
...
{
  TestAddIncremental<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                                        flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestAddIncremental2)
{
  TestAddIncremental2<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                                         flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestRemove)
{
  TestRemove<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                                flann::SearchParams(-1));
}

TEST_F(KDTreeSingle, TestSave)
{
  TestSave<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                              flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestSearchReorder)
{
  TestSearch<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, true), query, indices, dists, knn,
                                flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestSaveReorder)
{
  TestSave<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, true), query, indices, dists, knn,
                              flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestCopy)
{
  TestCopy<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, false), query, indices, dists, knn,
                              flann::SearchParams(-1), 0.99, gt_indices);

  // repeat tests with reorder=true
  TestCopy<L2_Simple<float> >(data, flann::KDTreeSingleIndexParams(12, true), query, indices, dists, knn,
                              flann::SearchParams(-1), 0.99, gt_indices);
}

TEST_F(KDTreeSingle, TestCopy2)
{
  TestCopy2<flann::KDTreeSingleIndex<L2_Simple<float> > >(data, flann::KDTreeSingleIndexParams(12, false), query,
                                                          indices, dists, knn, flann::SearchParams(-1), 0.99,
                                                          gt_indices);

  // repeat tests with reorder=true
  TestCopy2<flann::KDTreeSingleIndex<L2_Simple<float> > >(data, flann::KDTreeSingleIndexParams(12, true), query,
                                                          indices, dists, knn, flann::SearchParams(-1), 0.99,
                                                          gt_indices);
}
...
TEST_F(KDTreeSinglePointCloud, TestKNearestSearch)
{
...

  {
    ...
    for (std::multimap<float, int>::iterator it = sorted_brute_force_result.begin();
    ...
    // if (k_indices.size() != no_of_neighbors)  cerr << "Found "<<k_indices.size()<<" instead of "<<no_of_neighbors<<"
    // neighbors.\n";
