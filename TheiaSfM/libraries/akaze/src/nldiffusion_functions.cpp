...
{
/* ************************************************************************* */
void gaussian_2D_convolution(const RowMatrixXf& src, RowMatrixXf& dst, size_t ksize_x, size_t ksize_y, float sigma)
{
  GaussianBlur(src, sigma, &dst);
}

/* ************************************************************************* */
void image_derivatives_scharr(const RowMatrixXf& src, RowMatrixXf& dst, const size_t xorder, const size_t yorder)
{
  ScharrDerivative(src, xorder, yorder, 1.0, false, &dst);
}

/* ************************************************************************* */
void pm_g1(const RowMatrixXf& Lx, const RowMatrixXf& Ly, RowMatrixXf& dst, const float k)
{
...
/* ************************************************************************* */
void pm_g2(const RowMatrixXf& Lx, const RowMatrixXf& Ly, RowMatrixXf& dst, const float k)
{
  const float inv_k = 1.0 / (k * k);
...
/* ************************************************************************* */
void weickert_diffusivity(const RowMatrixXf& Lx, const RowMatrixXf& Ly, RowMatrixXf& dst, const float k)
{
...
/* ************************************************************************* */
void charbonnier_diffusivity(const RowMatrixXf& Lx, const RowMatrixXf& Ly, RowMatrixXf& dst, const float k)
{
  const float inv_k = 1.0 / (k * k);
  dst = 1.0 / ((1.0 + inv_k * (Lx.array().square() + Ly.array().square())).sqrt());
}

/* ************************************************************************* */
float compute_k_percentile(const RowMatrixXf& img, float perc, float gscale, size_t nbins, size_t ksize_x,
                           size_t ksize_y)
{
...
/* ************************************************************************* */
void compute_scharr_derivatives(const RowMatrixXf& src, RowMatrixXf& dst, const size_t xorder, const size_t yorder,
                                const size_t scale)
{
...
/* ************************************************************************* */
void nld_step_scalar(RowMatrixXf& Ld, const RowMatrixXf& c, RowMatrixXf& Lstep, const float stepsize)
{
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (int x = 1; x < Lstep.cols() - 1; x++)
  {
    const float xpos = (c(end_row, x) + c(end_row, x + 1)) * (Ld(end_row, x + 1) - Ld(end_row, x));
    const float xneg = (c(end_row, x - 1) + c(end_row, x)) * (Ld(end_row, x) - Ld(end_row, x - 1));
    const float ypos = (c(end_row, x) + c(end_row - 1, x)) * (Ld(end_row - 1, x) - Ld(end_row, x));
  ...
  for (int i = 1; i < Lstep.rows() - 1; i++)
  {
    ...
    const float xneg = (c(i, c_min_2) + c(i, c_min_1)) * (Ld(i, c_min_1) - Ld(i, c_min_2));
    ypos = (c(i, c_min_1) + c(i + 1, c_min_1)) * (Ld(i + 1, c_min_1) - Ld(i, c_min_1));
    yneg = (c(i - 1, c_min_1) + c(i, c_min_1)) * (Ld(i, c_min_1) - Ld(i - 1, c_min_1));
...
void halfsample_image(const RowMatrixXf& src, RowMatrixXf& dst)
{
  ...
  if (x_kernel_size == 2 && y_kernel_size == 2)
  {
    for (int i = 0; i < dst.rows(); i++)
    {
      ...
      {
        dst(i, j) = src(2 * i, 2 * j) + src(2 * i + 1, 2 * j) + src(2 * i, 2 * j + 1) + src(2 * i + 1, 2 * j + 1);
      }
    }
    dst /= 4.0;
    return;
...
#pragma omp parallel for firstprivate(temp_row, x_kernel_mul, y_kernel_mul)
  ...
  for (int i = 0; i < dst.rows(); i++)
  {
    ...
    y_kernel_mul(y_kernel_clamped_size - 1) -= y_kernel_mul.sum() - y_kernel_size;

    temp_row = y_kernel_mul * src.block(y, 0, y_kernel_clamped_size, src.cols());
    ...
    for (int j = 0; j < dst.cols(); j++)
    {
      ...
      dst(i, j) = x_kernel_mul.dot(temp_row.segment(x, x_kernel_clamped_size));
    }
  }
...
/* ************************************************************************* */
bool check_maximum_neighbourhood(const RowMatrixXf& img, int dsize, float value, int row, int col, bool same_img)
{
  bool response = true;
