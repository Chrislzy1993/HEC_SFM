...
int AKAZE::Create_Nonlinear_Scale_Space(const RowMatrixXf& img)
{
  ...
  options_.kcontrast = compute_k_percentile(img, options_.kcontrast_percentile, 1.0, options_.kcontrast_nbins, 0, 0);
...
void AKAZE::Compute_Determinant_Hessian_Response()
{
  ...
  for (size_t i = 0; i < evolution_.size(); i++)
  {
    ...
    {
      std::cout << "Computing detector response. Determinant of Hessian. Evolution "
                   "time: "
                << evolution_[i].etime << std::endl;
    }

    evolution_[i].Ldet = evolution_[i].Lxx.cwiseProduct(evolution_[i].Lyy).array() - evolution_[i].Lxy.array().square();
  }
}
...
void AKAZE::Find_Scale_Space_Extrema(std::vector<AKAZEKeypoint>& kpts)
{
  ...
  // Set maximum size
  if (options_.descriptor == SURF_UPRIGHT || options_.descriptor == SURF || options_.descriptor == MLDB_UPRIGHT ||
      options_.descriptor == MLDB)
  {
    smax = 10.0 * sqrtf(2.0);
  }
  ...
  for (size_t i = 0; i < evolution_.size(); i++)
  {
    for (int ix = 1; ix < evolution_[i].Ldet.rows() - 1; ix++)
    {
      for (int jx = 1; jx < evolution_[i].Ldet.cols() - 1; jx++)
      {
        ...
        if (value > options_.dthreshold && value >= options_.min_dthreshold && value > ldet(ix, jx - 1) &&
            value > ldet(ix, jx + 1) && value > ldet(ix - 1, jx - 1) && value > ldet(ix - 1, jx) &&
            value > ldet(ix - 1, jx + 1) && value > ldet(ix + 1, jx - 1) && value > ldet(ix + 1, jx) &&
            value > ldet(ix + 1, jx + 1))
        {
          is_extremum = true;
          ...
          {
            if ((point.class_id - 1) == kpts_aux[ik].class_id || point.class_id == kpts_aux[ik].class_id)
            {
          ...
          if (is_extremum == true)
          {
            ...
            if (left_x < 0 || right_x >= evolution_[i].Ldet.cols() || up_y < 0 || down_y >= evolution_[i].Ldet.rows())
            {
              is_out = true;
            }
...
void AKAZE::Do_Subpixel_Refinement(std::vector<AKAZEKeypoint>& kpts)
{
  ...
  for (size_t i = 0; i < kpts.size(); i++)
  {
    ...
    Dx = 0.5 * (evolution_[kpts[i].class_id].Ldet(y, x + 1) - evolution_[kpts[i].class_id].Ldet(y, x - 1));
    Dy = 0.5 * (evolution_[kpts[i].class_id].Ldet(y + 1, x) - evolution_[kpts[i].class_id].Ldet(y - 1, x));

    // Compute the Hessian
    Dxx = evolution_[kpts[i].class_id].Ldet(y, x + 1) + evolution_[kpts[i].class_id].Ldet(y, x - 1) -
          2.0 * evolution_[kpts[i].class_id].Ldet(y, x);

    Dyy = evolution_[kpts[i].class_id].Ldet(y + 1, x) + evolution_[kpts[i].class_id].Ldet(y - 1, x) -
          2.0 * evolution_[kpts[i].class_id].Ldet(y, x);

    Dxy = 0.25 * (evolution_[kpts[i].class_id].Ldet(y + 1, x + 1) + evolution_[kpts[i].class_id].Ldet(y - 1, x - 1)) -
          0.25 * (evolution_[kpts[i].class_id].Ldet(y - 1, x + 1) + evolution_[kpts[i].class_id].Ldet(y + 1, +x - 1));
...
*/
void AKAZE::Compute_Descriptors(std::vector<AKAZEKeypoint>& kpts, AKAZEDescriptors& desc)
{
...
#pragma omp parallel for
      ...
      for (int i = 0; i < (int)(kpts.size()); i++)
      {
        Get_Upright_MLDB_Full_Descriptor(kpts[i], (unsigned char*)(desc.binary_descriptor[i].data()));
      }
    }
    break;
    case MLDB:
...
#pragma omp parallel for
      ...
      {
        Compute_Main_Orientation(kpts[i]);
        Get_MLDB_Full_Descriptor(kpts[i], (unsigned char*)(desc.binary_descriptor[i].data()));
      }
    }
    break;
  }
...
void AKAZE::Compute_Main_Orientation(AKAZEKeypoint& kpt) const
{
  ...
  {
    ang2 = (ang1 + M_PI / 3.0f > 2.0 * M_PI ? ang1 - 5.0f * M_PI / 3.0f : ang1 + M_PI / 3.0f);
    sumX = sumY = 0.f;
    ...
    for (size_t k = 0; k < 109; ++k)
    {
      ...
      else if (ang2 < ang1 && ((ang > 0 && ang < ang2) || (ang > ang1 && ang < 2.0 * M_PI)))
      {
        sumX += resX[k];
        sumY += resY[k];
      }
...
/* ************************************************************************* */
void AKAZE::Get_SURF_Descriptor_Upright_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const
{
  desc.setZero(64);
...
/* ************************************************************************* */
void AKAZE::Get_SURF_Descriptor_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const
{
  desc.setZero(64);
  float dx = 0.0, dy = 0.0, mdx = 0.0, mdy = 0.0;
...
/* ************************************************************************* */
void AKAZE::Get_MSURF_Upright_Descriptor_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const
{
  desc.setZero(64);
...
/* ************************************************************************* */
void AKAZE::Get_MSURF_Descriptor_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const
{
  desc.setZero(64);
  float dx = 0.0, dy = 0.0, mdx = 0.0, mdy = 0.0, gauss_s1 = 0.0, gauss_s2 = 0.0;
...
/* ************************************************************************* */
void AKAZE::Get_MLDB_Full_Descriptor(const AKAZEKeypoint& kpt, unsigned char* desc) const
{
...
/* ************************************************************************* */
void AKAZE::Get_Upright_MLDB_Full_Descriptor(const AKAZEKeypoint& kpt, unsigned char* desc) const
{
  const int max_channels = 3;
  assert(options_.descriptor_channels <= max_channels);
  float values[16 * max_channels];
...
/* ************************************************************************* */
void AKAZE::MLDB_Fill_Values(float* values, int sample_step, int level, float xf, float yf, float co, float si,
                             float scale) const
{
  int pattern_size = options_.descriptor_pattern_size;
...
/* ************************************************************************* */
void AKAZE::MLDB_Fill_Upright_Values(float* values, int sample_step, int level, float xf, float yf, float scale) const
{
  int pattern_size = options_.descriptor_pattern_size;
  int nr_channels = options_.descriptor_channels;
  int valpos = 0;
...
/* ************************************************************************* */
void AKAZE::MLDB_Binary_Comparisons(float* values, unsigned char* desc, int count, int& dpos) const
{
...
/* ************************************************************************* */
void AKAZE::Get_MLDB_Descriptor_Subset(const AKAZEKeypoint& kpt, unsigned char* desc)
{
...
/* ************************************************************************* */
void AKAZE::Get_Upright_MLDB_Descriptor_Subset(const AKAZEKeypoint& kpt, unsigned char* desc)
{
...
/* ************************************************************************* */
void libAKAZE::generateDescriptorSubsample(Eigen::MatrixXi& sampleList, Eigen::MatrixXi& comparisons, int nbits,
                                           int pattern_size, int nchannels)
{
  ...
  srand(1024);
  Eigen::MatrixXi comps(nchannels * static_cast<int>(ceil(nbits / (float)nchannels)), 2);
