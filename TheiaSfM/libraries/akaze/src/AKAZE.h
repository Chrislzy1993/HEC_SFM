...
public:
  ...
  void Compute_Descriptors(std::vector<AKAZEKeypoint>& kpts, AKAZEDescriptors& desc);
  ...
  void Get_SURF_Descriptor_Upright_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const;
  ...
  // al.,
  // Speeded Up Robust Features, ECCV, 2006
  void Get_SURF_Descriptor_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const;
  ...
  // ECCV 2008
  void Get_MSURF_Upright_Descriptor_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const;
  ...
  // Feature Detection and Matching,
  // ECCV 2008
  void Get_MSURF_Descriptor_64(const AKAZEKeypoint& kpt, Eigen::VectorXf& desc) const;
  ...
  void Get_Upright_MLDB_Full_Descriptor(const AKAZEKeypoint& kpt, unsigned char* desc) const;

  // Computes the rotation invariant M-LDB binary descriptor (maximum
  // descriptor length)
  // @param kpt Input keypoint
  // @param desc Binary-based descriptor
  void Get_MLDB_Full_Descriptor(const AKAZEKeypoint& kpt, unsigned char* desc) const;
  /// Compute the upright (not rotation invariant) M-LDB binary descriptor
  /// (specified descriptor length)
  /// @param kpt Input keypoint
  /// @param desc Binary-based descriptor
  void Get_Upright_MLDB_Descriptor_Subset(const AKAZEKeypoint& kpt, unsigned char* desc);

  /// Computes the rotation invariant M-LDB binary descriptor (specified
  /// descriptor length)
  /// @param kpt Input keypoint
  /// @param desc Binary-based descriptor
  void Get_MLDB_Descriptor_Subset(const AKAZEKeypoint& kpt, unsigned char* desc);
  ...
  void MLDB_Fill_Upright_Values(float* values, int sample_step, int level, float xf, float yf, float scale) const;
...
void generateDescriptorSubsample(Eigen::MatrixXi& sampleList, Eigen::MatrixXi& comparisons, int nbits, int pattern_size,
                                 int nchannels);
