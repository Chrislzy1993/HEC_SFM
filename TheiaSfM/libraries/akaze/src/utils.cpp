...
void ConvertEigenToCImg(const RowMatrixXf& mat, cimg_library::CImg<float>& cimg)
{
...
void ConvertCImgToEigen(const cimg_library::CImg<float>& image, RowMatrixXf& eigen_image)
{
  ...
  eigen_image = Eigen::Map<const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> >(
      grayscale_image.data(), grayscale_image.height(), grayscale_image.width());
}
...
// numbers.
int HammingDistance(const libAKAZE::BinaryVectorX& desc1, const libAKAZE::BinaryVectorX& desc2)
{
...
void match_features(const std::vector<Eigen::VectorXf>& desc1, const std::vector<Eigen::VectorXf>& desc2,
...
#pragma omp parallel for
  ...
  for (int i = 0; i < match_distances.size(); i++)
  {
    ...
    std::partial_sort(match_distances[i].begin(), match_distances[i].begin() + 2, match_distances[i].end());
...
void match_features(const std::vector<libAKAZE::BinaryVectorX>& desc1,
                    const std::vector<libAKAZE::BinaryVectorX>& desc2, const double ratio,
...
#pragma omp parallel for
  ...
  for (int i = 0; i < match_distances.size(); i++)
  {
    // Get the top 2 matches.
    std::partial_sort(match_distances[i].begin(), match_distances[i].begin() + 2, match_distances[i].end());
    if (static_cast<double>(match_distances[i][0].first) / match_distances[i][1].first < ratio)
    {
      matches.push_back(std::make_pair(i, match_distances[i][0].second));
    }
  }
...
void compute_inliers_homography(const std::vector<libAKAZE::AKAZEKeypoint>& kpts1,
                                const std::vector<libAKAZE::AKAZEKeypoint>& kpts2,
                                const std::vector<std::pair<int, int> >& matches,
                                std::vector<std::pair<int, int> >& inliers, const Eigen::Matrix3f& H, float min_error)
{
...
/* ************************************************************************* */
void draw_keypoints(cimg_library::CImg<float>& image, const std::vector<libAKAZE::AKAZEKeypoint>& kpts)
{
  int x = 0, y = 0;
  float radius = 0.0;
...
void draw_matches(cimg_library::CImg<float>& image1, cimg_library::CImg<float>& image2,
                  const std::vector<libAKAZE::AKAZEKeypoint>& kpts1, const std::vector<libAKAZE::AKAZEKeypoint>& kpts2,
                  const std::vector<std::pair<int, int> >& matches, cimg_library::CImg<float>& matched_image)
{
  ...
  for (int i = 0; i < matches.size(); i++)
  {
    ...
    matched_image.draw_line(pt1.x(), pt1.y(), pt2.x() + image1.width(), pt2.y(), color);
  }
}

/* ************************************************************************* */
int save_keypoints(const std::string& outFile, const std::vector<libAKAZE::AKAZEKeypoint>& kpts,
                   const std::vector<Eigen::VectorXf>& desc, bool save_desc)
{
  if (kpts.size() == 0)
  {
...
int save_keypoints(const std::string& outFile, const std::vector<libAKAZE::AKAZEKeypoint>& kpts,
                   const std::vector<libAKAZE::BinaryVectorX>& desc, bool save_desc)
{
  if (kpts.size() == 0)
  {
    std::cerr << "No keypoints exist." << std::endl;
...
void show_input_options_help(int example)
{
  ...
  {
    std::cout << "./akaze_match img1.jpg img2.pgm homography.txt output.jpg [options]" << std::endl;
  }

  std::cout << std::endl;
  cout_help() << "homography.txt is optional for image matching. If it is "
                 "present, only the inliers are shown."
              << std::endl;
