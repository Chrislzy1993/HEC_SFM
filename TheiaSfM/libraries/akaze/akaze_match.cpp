 ...
 * homography
 */
int parse_input_options(libAKAZE::AKAZEOptions& options, std::string& img_path1, std::string& img_path2,
...
int main(int argc, char* argv[])
{
  ...
  {
    libAKAZE::match_features(desc1.float_descriptor, desc2.float_descriptor, DRATIO, matches);
  }
  // Binary descriptor, use Hamming distance
  else
  {
    libAKAZE::match_features(desc1.binary_descriptor, desc2.binary_descriptor, DRATIO, matches);
  }
  ...
  cimg_library::CImg<float> img1_rgb = img1.get_resize(img1.width(), img1.height(), img1.depth(), 3);
  cimg_library::CImg<float> img2_rgb = img2.get_resize(img2.width(), img2.height(), img2.depth(), 3);
