...
VL_EXPORT T VL_XCAT(_vl_distance_l2_, SFX)(vl_size dimension, T const* X, T const* Y)
{
...
VL_EXPORT T VL_XCAT(_vl_distance_l1_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_distance_chi2_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_distance_hellinger_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_distance_js_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
  {
    ...
    T y = *Y++;
    if (x)
      acc += x - x * VL_XCAT(vl_log2_, SFX)(1 + y / x);
    if (y)
      acc += y - y * VL_XCAT(vl_log2_, SFX)(1 + x / y);
  }
  return acc;
}

VL_EXPORT T VL_XCAT(_vl_kernel_l2_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_kernel_l1_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_kernel_chi2_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_kernel_hellinger_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT T VL_XCAT(_vl_kernel_js_, SFX)(vl_size dimension, T const* X, T const* Y)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
  {
    ...
    T y = *Y++;
    if (x)
      acc += x * VL_XCAT(vl_log2_, SFX)(1 + y / x);
    if (y)
      acc += y * VL_XCAT(vl_log2_, SFX)(1 + x / y);
  }
  return (T)0.5 * acc;
}

VL_EXPORT T VL_XCAT(_vl_distance_mahalanobis_sq_, SFX)(vl_size dimension, T const* X, T const* MU, T const* S)
{
  T const* X_end = X + dimension;
  T acc = 0.0;
  while (X < X_end)
...
VL_EXPORT COMPARISONFUNCTION_TYPE VL_XCAT(vl_get_vector_comparison_function_, SFX)(VlVectorComparisonType type)
{
...
VL_EXPORT COMPARISONFUNCTION3_TYPE VL_XCAT(vl_get_vector_3_comparison_function_, SFX)(VlVectorComparisonType type)
{
...
VL_EXPORT void VL_XCAT(vl_eval_vector_comparison_on_all_pairs_, SFX)(T* result, vl_size dimension, T const* X,
                                                                     vl_size numDataX, T const* Y, vl_size numDataY,
...
void vl_svd2(double* S, double* U, double* V, double const* M)
{
  double m11 = M[0];
  double m21 = M[1];
  double m12 = M[2];
...
void vl_lapack_dlasv2(double* smin, double* smax, double* sv, double* cv, double* su, double* cu, double f, double g,
                      double h)
{
  ...
  if (fa < ha)
  {
    ...
    tmp = ft;
    ft = ht;
    ht = tmp; /* swap ft and ht */
    tmp = fa;
    fa = ha;
    ha = tmp; /* swap fa and ha */
    swap = 1;
  }
...
VL_EXPORT vl_bool vl_gaussian_elimination(double* A, vl_size numRows, vl_size numColumns)
{
  vl_index i, j, ii, jj;
  assert(A);
