...
static void _vl_sift_smooth(VlSiftFilt* self, vl_sift_pix* outputImage, vl_sift_pix* tempImage,
                            vl_sift_pix const* inputImage, vl_size width, vl_size height, double sigma)
{
  ...
  vl_imconvcol_vf(tempImage, height, inputImage, width, height, width, self->gaussFilter, -self->gaussFilterWidth,
                  self->gaussFilterWidth, 1, VL_PAD_BY_CONTINUITY | VL_TRANSPOSE);

  vl_imconvcol_vf(outputImage, width, tempImage, height, width, height, self->gaussFilter, -self->gaussFilterWidth,
                  self->gaussFilterWidth, 1, VL_PAD_BY_CONTINUITY | VL_TRANSPOSE);
}
...
int vl_sift_process_first_octave(VlSiftFilt* f, vl_sift_pix const* im)
{
  ...
  for (s = s_min + 1; s <= s_max; ++s)
  {
    ...
    _vl_sift_smooth(f, vl_sift_get_octave(f, s), temp, vl_sift_get_octave(f, s - 1), w, h, sd);
  }
...
#define CHECK_NEIGHBORS(CMP, SGN)                                                                                      \
  (v CMP## = SGN 0.8 * tp && v CMP * (pt + xo) && v CMP * (pt - xo) && v CMP * (pt + so) && v CMP * (pt - so) &&       \
             v CMP * (pt + yo) && v CMP * (pt - yo) &&                                                                 \
                                                                                                                       \
             v CMP * (pt + yo + xo) && v CMP * (pt + yo - xo) && v CMP * (pt - yo + xo) && v CMP * (pt - yo - xo) &&   \
                                                                                                                       \
             v CMP * (pt + xo + so) && v CMP * (pt - xo + so) && v CMP * (pt + yo + so) && v CMP * (pt - yo + so) &&   \
             v CMP * (pt + yo + xo + so) && v CMP * (pt + yo - xo + so) && v CMP * (pt - yo + xo + so) &&              \
             v CMP * (pt - yo - xo + so) &&                                                                            \
                                                                                                                       \
             v CMP * (pt + xo - so) && v CMP * (pt - xo - so) && v CMP * (pt + yo - so) && v CMP * (pt - yo - so) &&   \
             v CMP * (pt + yo + xo - so) && v CMP * (pt + yo - xo - so) && v CMP * (pt - yo + xo - so) &&              \
        ...
        if (CHECK_NEIGHBORS(>, +) || CHECK_NEIGHBORS(<, -))
        {
          ...
          if (f->nkeys >= f->keys_res)
          {
            ...
            if (f->keys)
            {
              f->keys = vl_realloc(f->keys, f->keys_res * sizeof(VlSiftKeypoint));
            }
            else
            {
...
#define Aat(i, j) (A[(i) + (j)*3])
    ...
    /* check threshold and other conditions */
    {
      ...
      vl_bool good = vl_abs_d(val) > tp && score < (te + 1) * (te + 1) / te && score >= 0 && vl_abs_d(b[0]) < 1.5 &&
                     vl_abs_d(b[1]) < 1.5 && vl_abs_d(b[2]) < 1.5 && xn >= 0 && xn <= w - 1 && yn >= 0 && yn <= h - 1 &&
                     sn >= s_min && sn <= s_max;
...
int vl_sift_calc_keypoint_orientations(VlSiftFilt* f, double angles[4], VlSiftKeypoint const* k)
{
  ...
  if (xi < 0 || xi > w - 1 || yi < 0 || yi > h - 1 || si < f->s_min + 1 || si > f->s_max - 2)
  {
    return 0;
  }
...
VL_EXPORT
void vl_sift_calc_raw_descriptor(VlSiftFilt const* f, vl_sift_pix const* grad, vl_sift_pix* descr, int width,
                                 int height, double x, double y, double sigma, double angle0)
{
...
#define atd(dbinx, dbiny, dbint) *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
  ...
  for (dyi = VL_MAX(-W, -yi); dyi <= VL_MIN(+W, h - yi - 1); ++dyi)
  {
    for (dxi = VL_MAX(-W, -xi); dxi <= VL_MIN(+W, w - xi - 1); ++dxi)
    {
      ...
      vl_sift_pix win = fast_expn((nx * nx + ny * ny) / (2.0 * wsigma * wsigma));
      ...
      for (dbinx = 0; dbinx < 2; ++dbinx)
      {
        for (dbiny = 0; dbiny < 2; ++dbiny)
        {
          ...
          {
            if (binx + dbinx >= -(NBP / 2) && binx + dbinx < (NBP / 2) && biny + dbiny >= -(NBP / 2) &&
                biny + dbiny < (NBP / 2))
            {
              vl_sift_pix weight =
                  win * mod * vl_abs_f(1 - dbinx - rbinx) * vl_abs_f(1 - dbiny - rbiny) * vl_abs_f(1 - dbint - rbint);
  ...
  if (1)
  {
     ...
     */
    int numSamples = (VL_MIN(W, w - xi - 1) - VL_MAX(-W, -xi) + 1) * (VL_MIN(W, h - yi - 1) - VL_MAX(-W, -yi) + 1);
...
void vl_sift_calc_keypoint_descriptor(VlSiftFilt* f, vl_sift_pix* descr, VlSiftKeypoint const* k, double angle0)
{
  ...
  */
  ...
  /* check bounds */
  if (k->o != f->o_cur || xi < 0 || xi >= w || yi < 0 || yi >= h - 1 || si < f->s_min + 1 || si > f->s_max - 2)
    return;
