...
@sa
http://software.intel.com/sites/products/documentation/doclib/mkl_sa/11/mkl_userguide_win/GUID-C2295BC8-DD22-466B-94C9-5FAA79D4F56D.htm
...
char* vl_configuration_to_string_copy()
{
  ...
  while (string == 0)
  {
    ...
    length = snprintf(string, length, "VLFeat version %s\n"
                                      "    Static config: %s\n"
                                      "    %" VL_FMT_SIZE " CPU(s): %s\n"
#if defined(_OPENMP)
...
#endif
                                      "    Debug: %s\n",
                      vl_get_version_string(), staticString, vl_get_num_cpus(), cpuString,
#if defined(_OPENMP)
...
static void vl_lock_state(void)
{
  ...
  if (state->mutexCount >= 1 && pthread_equal(state->mutexOwner, thisThread))
  {
    state->mutexCount++;
  }
  else
...
#else
    vsnprintf(state->lastErrorMessage, sizeof(state->lastErrorMessage) / sizeof(char), errorMessage, args);
#endif
  }
  else
  {
...
void vl_set_alloc_func(void* (*malloc_func)(size_t), void* (*realloc_func)(void*, size_t),
...
double vl_toc(void)
{
  ...
  return (double)(tocMark.QuadPart - threadState->ticMark.QuadPart) / threadState->ticFreq.QuadPart;
#else
...
static void vl_constructor(void)
{
...
#if defined(VL_THREADS_POSIX)
  {
    ...
    pthread_key_create(&state->threadKey, (destructorType)vl_thread_specific_state_delete);
...
static void vl_destructor()
{
...
#if defined(VL_THREADS_POSIX)
  {
     ...
     */
    VlThreadState* threadState = pthread_getspecific(state->threadKey);
    if (threadState)
    {
      vl_thread_specific_state_delete(threadState);
...
#elif defined(VL_THREADS_WIN)
  {
    /* Delete the thread state of this thread as the
       ...
       using the library, so this is fine.
     */
    VlThreadState* threadState = TlsGetValue(state->tlsIndex);
    if (threadState)
    {
      vl_thread_specific_state_delete(threadState);
