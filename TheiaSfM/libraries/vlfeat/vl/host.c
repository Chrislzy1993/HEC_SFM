...
#include <intrin.h>
VL_INLINE void _vl_cpuid(vl_int32* info, int function)
{
...
VL_INLINE void _vl_cpuid(vl_int32* info, int function)
{
  ...
  __asm__ __volatile__("cpuid" : "=a"(info[0]), "=b"(info[1]), "=c"(info[2]), "=d"(info[3]) : "a"(function) : "cc");
#endif
}
...
char* _vl_x86cpu_info_to_string_copy(VlX86CpuInfo const* self)
{
  ...
  while (string == 0)
  {
    ...
    length = snprintf(string, length, "%s%s%s%s%s%s%s%s", self->vendor.string, self->hasMMX ? " MMX" : "",
                      self->hasSSE ? " SSE" : "", self->hasSSE2 ? " SSE2" : "", self->hasSSE3 ? " SSE3" : "",
                      self->hasSSE41 ? " SSE41" : "", self->hasSSE42 ? " SSE42" : "", self->hasAVX ? " AVX" : "");
    length += 1;
  }
  return string;
}
...
#define v VL_COMPILER_GNUC
...

  {
    ...
    while (string == 0)
    {
      ...
      length = snprintf(string, length, "%s, %s, %s", hostString, compilerString, libraryString);
      length += 1;
    }
    return string;
  }
