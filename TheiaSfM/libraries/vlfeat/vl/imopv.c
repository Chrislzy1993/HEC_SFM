...
/** @fn vl_imconvcol_vd(double*,vl_size,double const*,vl_size,vl_size,vl_size,double
 *const*,vl_index,vl_index,int,unsigned int)
...
/** @fn vl_imconvcol_vf(float*,vl_size,float const*,vl_size,vl_size,vl_size,float const*,vl_index,vl_index,int,unsigned
 *int)
 ** @see ::vl_imconvcol_vd
 **/

VL_EXPORT void VL_XCAT(vl_imconvcol_v, SFX)(T* dst, vl_size dst_stride, T const* src, vl_size src_width,
                                            vl_size src_height, vl_size src_stride, T const* filt, vl_index filt_begin,
                                            vl_index filt_end, int step, unsigned int flags)
{
  vl_index x = 0;
  vl_index y;
...
/* dispatch to accelerated version */
  ...
  if (vl_cpu_has_sse2() && vl_get_simd_enabled())
  {
    ...
    (dst, dst_stride, src, src_width, src_height, src_stride, filt, filt_begin, filt_end, step, flags);
    return;
  }
#endif
...
VL_EXPORT void VL_XCAT(vl_image_distance_transform_, SFX)(T const* image, vl_size numColumns, vl_size numRows,
                                                          vl_size columnStride, vl_size rowStride, T* distanceTransform,
                                                          vl_uindex* indexes, T coeff, T offset)
{
...
/* If coeff is very small, the parabolas are flat (= lines).
 ...
 pixel has lower score */
      ...
      base[num] = r;
      if (indexes)
        baseIndexes[num] = indexes[x * columnStride + y * rowStride];
      num++;
    } /* next column */
    ...
    for (x = 0; x < numColumns; ++x)
    {
      ...
      distanceTransform[x * columnStride + y * rowStride] = base[num] + coeff * delta * delta;
      if (indexes)
      {
        indexes[x * columnStride + y * rowStride] = baseIndexes[num];
      }
    }
  } /* next row */
...
VL_EXPORT void VL_XCAT(vl_imconvcoltri_, SFX)(T* dest, vl_size destStride, T const* image, vl_size imageWidth,
                                              vl_size imageHeight, vl_size imageStride, vl_size filterSize,
                                              vl_size step, unsigned int flags)
{
  ...
  while (x < (signed)imageWidth)
  {
    ...
    for (y = -(signed)filterSize; y < (signed)imageHeight - (signed)filterSize; ++y)
    {
      buffer[y] = buffer[y] - buffer[y + filterSize];
    }
    if (!zeropad)
    {
      for (y = (signed)imageHeight - (signed)filterSize; y < (signed)imageHeight; ++y)
      {
        buffer[y] = buffer[y] - buffer[imageHeight - 1] * ((signed)imageHeight - (signed)filterSize - y);
      }
    }

    /* integrate forward the column */
    for (y = -(signed)filterSize + 1; y < (signed)imageHeight; ++y)
    {
      buffer[y] += buffer[y - 1];
    }
...
static T* VL_XCAT(_vl_new_gaussian_fitler_, SFX)(vl_size* size, double sigma)
{
  T* filter;
  T mass = (T)1.0;
  vl_index i;
...
VL_EXPORT void VL_XCAT(vl_imsmooth_, SFX)(T* smoothed, vl_size smoothedStride, T const* image, vl_size width,
                                          vl_size height, vl_size stride, double sigmax, double sigmay)
{
  ...
  (buffer, height, image, width, height, stride, filtery, -((signed)sizey - 1) / 2, ((signed)sizey - 1) / 2, 1,
   VL_PAD_BY_CONTINUITY | VL_TRANSPOSE);

  VL_XCAT(vl_imconvcol_v, SFX)
  (smoothed, smoothedStride, buffer, height, width, height, filterx, -((signed)sizex - 1) / 2, ((signed)sizex - 1) / 2,
...
VL_EXPORT void VL_XCAT(vl_imgradient_, SFX)(T* xGradient, T* yGradient, vl_size gradWidthStride,
                                            vl_size gradHeightStride, T const* image, vl_size imageWidth,
...
VL_EXPORT void VL_XCAT(vl_imgradient_polar_, SFX)(T* gradientModulus, T* gradientAngle,
                                                  vl_size gradientHorizontalStride, vl_size gradHeightStride,
                                                  T const* image, vl_size imageWidth, vl_size imageHeight,
                                                  vl_size imageStride)
{
  /* Shortcuts */
  vl_index const xo = 1;
...
VL_EXPORT void VL_XCAT(vl_imintegral_, SFX)(T* integral, vl_size integralStride, T const* image, vl_size imageWidth,
                                            vl_size imageHeight, vl_size imageStride)
{
  vl_uindex x, y;
  T temp = 0;
