...
static union
{
  vl_uint32 raw;
  float value;
} const vl_infinity_f = { 0x7F800000UL };
...
#define VL_FAST_SQRT_UI(T, SFX)                                                                                        \
  VL_INLINE T vl_fast_sqrt_##SFX(T x)                                                                                  \
  {                                                                                                                    \
    T y = 0;                                                                                                           \
    T tmp = 0;                                                                                                         \
    int twice_k;                                                                                                       \
    for (twice_k = 8 * sizeof(T) - 2; twice_k >= 0; twice_k -= 2)                                                      \
    {                                                                                                                  \
      y <<= 1; /* y = 2 * y */                                                                                         \
      tmp = (2 * y + 1) << twice_k;                                                                                    \
      if (x >= tmp)                                                                                                    \
...
VL_EXPORT void vl_eval_vector_comparison_on_all_pairs_f(float* result, vl_size dimension, float const* X,
                                                        vl_size numDataX, float const* Y, vl_size numDataY,
                                                        VlFloatVectorComparisonFunction function);

VL_EXPORT void vl_eval_vector_comparison_on_all_pairs_d(double* result, vl_size dimension, double const* X,
                                                        vl_size numDataX, double const* Y, vl_size numDataY,
...
VL_EXPORT void vl_lapack_dlasv2(double* smin, double* smax, double* sv, double* cv, double* su, double* cu, double f,
                                double g, double h);
