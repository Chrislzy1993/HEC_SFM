...
void eigs_sym_F77(MatrixXd& M, VectorXd& init_resid, int k, int m, double& time_used, double& prec_err)
{
  ...
  seupd(rvec, howmny, d, Z, ldz, sigma, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam, ipntr, workd, workl,
        lworkl, ierr);
...
void eigs_gen_F77(MatrixXd& M, VectorXd& init_resid, int k, int m, double& time_used, double& prec_err)
{
  ...
  naupd(ido, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam, ipntr, workd, workl, lworkl, info);
  // ido == -1 or ido == 1 means more iterations needed
  while (ido == -1 || ido == 1)
  {
    MapVec vec_in(&workd[ipntr[0] - 1], n);
  ...
  // Use seupd() to retrieve results
  neupd(rvec, howmny, dr, di, Z, ldz, sigmar, sigmai, workv, bmat, n, which, nev, tol, resid, ncv, V, ldv, iparam,
