...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, TestName, Desc)                                                     \
  ...
  Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                                                            \
      &INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____)::test, #ClassName, Catch::NameAndDesc(TestName, Desc), \
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                                                        \
  ...
  } while (Catch::isTrue(false && (expr)))  // expr here is never evaluated at runtime but it forces the compiler to
                                            // give it a look
...
{
  explicit ReporterConfig(Ptr<IConfig> const& _fullConfig) : m_stream(&_fullConfig->stream()), m_fullConfig(_fullConfig)
  {
  }

  ReporterConfig(Ptr<IConfig> const& _fullConfig, std::ostream& _stream) : m_stream(&_stream), m_fullConfig(_fullConfig)
  {
  }
...
TestCaseInfo::TestCaseInfo(std::string const& _name, std::string const& _className, std::string const& _description,
                           std::set<std::string> const& _tags, SourceLineInfo const& _lineInfo)
  : name(_name), className(_className), description(_description), tags(_tags), lineInfo(_lineInfo), properties(None)
{
  std::ostringstream oss;
...
#define CATCH_REQUIRE_FALSE(expr)                                                                                      \
  INTERNAL_CATCH_TEST(expr, Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, "CATCH_REQUIRE_"   \
                                                                                                    "FALSE")
