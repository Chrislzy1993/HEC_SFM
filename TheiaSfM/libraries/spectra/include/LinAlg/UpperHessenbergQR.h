...
public:
  ...
  ///
  UpperHessenbergQR() : n(0), computed(false)
  {
  }
  ...
  UpperHessenbergQR(ConstGenericMatrix& mat)
    : n(mat.rows()), mat_T(n, n), rot_cos(n - 1), rot_sin(n - 1), computed(false)
  {
    compute(mat);
  }
  ...
  virtual Matrix matrix_RQ()
  {
    ...
    Scalar *c = rot_cos.data(), *s = rot_sin.data();
    for (int i = 0; i < n - 1; i++)
    {
  ...
  void apply_QY(GenericMatrix Y)
  {
    ...
    Scalar *c = rot_cos.data() + n - 2, *s = rot_sin.data() + n - 2;
...
public:
  ...
  /// be performed later by calling the compute() method.
  ///
  TridiagQR() : UpperHessenbergQR<Scalar>()
  {
  }
  ...
  /// the matrix are used.
  ///
  TridiagQR(ConstGenericMatrix& mat) : UpperHessenbergQR<Scalar>()
  {
    this->compute(mat);
  }
  ...
  void compute(ConstGenericMatrix& mat)
  {
    ...
    Scalar *Tii = this->mat_T.data(),       // pointer to T[i, i]
        *ptr,                               // some location relative to Tii
        ...
        r, tmp, eps = std::numeric_limits<Scalar>::epsilon();
  ...
  Matrix matrix_RQ()
  {
    ...
    // [m21  m22]
    Scalar *m11 = RQ.data(), *m12, *m21, *m22, *c = this->rot_cos.data(), *s = this->rot_sin.data(), tmp;
