...
private:
  ...
  int nev_adjusted(int nconv)
  {
    ...
    for (int i = nev; i < ncv; i++)
      if (std::abs(ritz_est[i]) < prec)
        nev_new++;

    // Increase nev by one if ritz_val[nev - 1] and
    // ritz_val[nev] are conjugate pairs
    if (is_complex(ritz_val[nev_new - 1], prec) && is_conj(ritz_val[nev_new - 1], ritz_val[nev_new], prec))
    {
      nev_new++;
    }
...
public:
  ...
  GenEigsRealShiftSolver(OpType* op_, int nev_, int ncv_, Scalar sigma_)
    : GenEigsSolver<Scalar, SelectionRule, OpType>(op_, nev_, ncv_), sigma(sigma_)
  {
...
private:
  ...
  void sort_ritzpair(int sort_rule)
  {
    ...
    for (int i = 0; i < this->nev; i++)
    {
      ...
      Complex lambdai = Complex(v_real[0], v_imag[0]) / Complex(lhs_real[0], lhs_imag[0]) + Complex(r, 0);
...
public:
  ...
  GenEigsComplexShiftSolver(OpType* op_, int nev_, int ncv_, Scalar sigmar_, Scalar sigmai_)
    : GenEigsSolver<Scalar, SelectionRule, OpType>(op_, nev_, ncv_), sigmar(sigmar_), sigmai(sigmai_)
  {
