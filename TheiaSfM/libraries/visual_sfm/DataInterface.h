...
struct CameraT_
{
  ...
  {
    constant_camera = 1.0f;
  }
  void SetVariableCamera()
  {
    constant_camera = 0.0f;
  }
  void SetFixedIntrinsic()
  {
    constant_camera = 2.0f;
  }
  ...
  template <class Float>
  void SetMeasumentDistortion(Float r)
  {
    radial = (float_t)r;
    distortion_type = -1;
  }
  ...
  float_t GetProjectionDistortion()
  {
    return distortion_type == 1 ? radial : 0;
  }
  ...
  template <class Float>
  void GetCameraCenter(Float c[3]) const
  {
    // C = - R' * t
    for (int j = 0; j < 3; ++j)
