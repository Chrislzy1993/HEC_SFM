...
bool LoadModelFile(const char* name, vector<CameraT>& camera_data, vector<Point3D>& point_data,
                   vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx, vector<string>& names,
                   vector<int>& ptc);
...
void AddStableNoise(vector<CameraT>& camera_data, vector<Point3D>& point_data, const vector<int>& ptidx,
                    const vector<int>& camidx, float percent);
bool RemoveInvisiblePoints(vector<CameraT>& camera_data, vector<Point3D>& point_data, vector<int>& ptidx,
                           vector<int>& camidx, vector<Point2D>& measurements, vector<string>& names, vector<int>& ptc);

/////////////////////////////////////////////////////////////////////////////
bool LoadNVM(ifstream& in, vector<CameraT>& camera_data, vector<Point3D>& point_data, vector<Point2D>& measurements,
             vector<int>& ptidx, vector<int>& camidx, vector<string>& names, vector<int>& ptc)
{
             ...
             vector<int>& ptc)
{
  ...
  if (ptc.size() < 3 * point_data.size())
    ptc.resize(point_data.size() * 3, 0);
  ...
  for (size_t i = 0; i < camera_data.size(); ++i)
  {
    ...
    for (int j = 0; j < 9; ++j)
      out << cam.m[0][j] << ' ';
    out << cam.t[0] << ' ' << cam.t[1] << ' ' << cam.t[2] << ' ' << cam.GetNormalizedMeasurementDistortion() << " 0\n";
  }
  ...
  for (size_t i = 0, j = 0; i < point_data.size(); ++i)
  {
    ...
    int* pc = &ptc[i * 3];
    out << pt.xyz[0] << ' ' << pt.xyz[1] << ' ' << pt.xyz[2] << ' ' << pc[0] << ' ' << pc[1] << ' ' << pc[2] << ' ';

    size_t je = j;
    while (je < ptidx.size() && ptidx[je] == (int)i)
      je++;

    out << (je - j) << ' ';

    for (; j < je; ++j)
      out << camidx[j] << ' ' << " 0 " << measurements[j].x << ' ' << measurements[j].y << ' ';
                    ...
                    vector<int>& ptc)
{
  ...
  for (size_t i = 0, j = 0; i < point_data.size(); ++i)
  {
    ...
    Point3D& pt = point_data[i];
    while (j + npj < point_data.size() && ptidx[j + npj] == ptidx[j])
      npj++;
    ///////////////////////////
    ...
    out << npj << ' ';
    for (int k = 0; k < npj; ++k)
      out << camidx[j + k] << " 0 " << measurements[j + k].x << ' ' << -measurements[j + k].y << '\n';
    out << '\n';
    j += npj;
  }
                      ...
                      vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx)
{
  ...
  for (size_t i = 0; i < camera_data.size(); ++i)
  {
    ...
    out << r[0] << ' ' << r[1] << ' ' << r[2] << ' ' << t[0] << ' ' << t[1] << ' ' << t[2] << ' ' << cam.f << ' '
                   ...
                   vector<int>& ptc)
{
  ...
  if (strstr(name, ".nvm"))
    return LoadNVM(in, camera_data, point_data, measurements, ptidx, camidx, names, ptc);
  else if (strstr(name, ".out"))
    return LoadBundlerOut(name, in, camera_data, point_data, measurements, ptidx, camidx, names, ptc);
  else
    return LoadBundlerModel(in, camera_data, point_data, measurements, ptidx, camidx);
}
                    ...
                    const vector<int>& camidx, float percent)
{
  ...
  {
    if (invalid_count)
      std::cout << "NOTE" << std::setw(2) << modify_iteration << ": modify " << invalid_count << " camera to fix "
                                                                                                 "visibility\n";

    //////////////////////////////////////////////////////
    for (size_t i = 0; i < camera_data.size(); ++i)
    {
      if (!need_modification[i])
        continue;
      CameraT& cam = camera_data[i];
      double e[3], c[3];
      cam = backup[i];
    ...
    {
      int cid = camidx[i];
      if (need_modification[cid] == false)
        continue;
      if (invalidc[cid])
        continue;
...
void ExamineVisiblity(const char* input_filename)
{
  ...
  for (size_t i = 0; i < ptidx.size(); ++i)
  {
    ...
    double fz2 =
        camt.m[2][0] * (ptt.xyz[0] - c[0]) + camt.m[2][1] * (ptt.xyz[1] - c[1]) + camt.m[2][2] * (ptt.xyz[2] - c[2]);
                           ...
                           vector<int>& camidx, vector<Point2D>& measurements, vector<string>& names, vector<int>& ptc)
{
  ...
  for (size_t i = 0; i < ptidx.size(); ++i)
  {
    ...
    bool visible =
        (cam.m[2][0] * pt.xyz[0] + cam.m[2][1] * pt.xyz[1] + cam.m[2][2] * pt.xyz[2] + cam.t[2] > dist_threshold);
  ...
  do
  {
    ...
    for (size_t i = 0; i < ptidx.size(); ++i)
    {
      int cid = camidx[i], pid = ptidx[i];
      if (!pmask[pid])
        continue;                     // point already removed
  ...
  std::cout << "NOTE: removing " << (camera_data.size() - new_camera_count) << " cameras; "
            << (point_data.size() - new_point_count) << " 3D Points; " << (measurements.size() - new_observation_count)
            << " Observations;\n";
