...
int MatchFile::MakeWritable()
{
  ...
  close(_fid);
  _fid = open(match_path, O_BINARY | O_CREAT | O_RDWR, S_IWRITE | S_IREAD);
  if (_fid > 0)
  {
    _opened_forwrite = 1;
    return 1;
...
int MatchFile::OpenMatchFile(const char* imagepath, int write, int mode)
{
  ...
  if (_fid > 0)
  {
    ...
    if (sizeof(_header) == readsz && _header.definition_size > 0 && _header.definition_buf >= _header.definition_size &&
...
int MatchFile::GetImageIndex(const char* relative_path)
{
  ...
  else
  {
    for (int i = 0; i < _match_records.size(); i++)
    {
      if (strcmp(_match_records[i]->file_name, relative_path) == 0)
        return i;
    }
  }
  return -1;
...
{
  int i = 0;
  while (path2[i] == path1[i] && path2[i] && path1[i])
    i++;
  while (i > 0 && path1[i] != PATH_SLASH)
    i--;
  if (i == 0)
  {
...
{
  if (i >= _header.file_count - 1 || i < 0 || _header.file_count < 2)
    return;
  ...
  // temp.begin() + len);
  _record_definition.insert(_record_definition.begin() + _header.definition_size - len, temp.begin(),
                            temp.begin() + len);
  ...
  else
  {
    ...
    if (match_move_needed < _header.file_count)
    {
      ...
      std::copy(temp.begin(), temp.begin() + definition_move,
                _record_definition.begin() + _header.definition_size - definition_move);
    }
...
void MatchFile::CloseMatchFile()
{
  if (_fid > 0)
  {
    if (_opened_forwrite && delay_header_update)
      UpdateHeaderAndRecordLoc();
    ResetMatchFile();
    close(_fid);
    _fid = 0;
...
void MatchFile::WriteIMatch(MatchFile* mat, const char* image1, const char* image2, int NM, TwoViewGeometry& tvg,
                            Points<int>& inliers)
{
...
void MatchFile::WriteIMatch(const char* image_match, int reverse, int NM, TwoViewGeometry& tvg, Points<int>& inliers)
{
  int index, fnm, szmin;
  char relative_path[MAX_PATH];
  if (image_match == NULL || image_match[0] == 0)
    return;
...
void MatchFile::WritePMatch(const char* image_match, int FC, int NM, Points<int>& matches, int reverse)
{
  ...
  index = GetImageIndex(relative_path);
  if (index < 0)
    index = AddImageMatch(relative_path);
  RecordLoc* loc = _match_records[index];
  loc->fcount = FC;
  assert(loc->read_loc > 0);
  size_t offset = ((char*)loc) - ((char*)_match_records[0]);
