...
void FeatureData::saveSIFTB2(const char* szFile)
{
  ...
  int fd = open(szFile, O_BINARY | O_CREAT | O_WRONLY | O_TRUNC, S_IREAD | S_IWRITE);
  if (fd < 0)
    return;
...
int FeatureData::OpenSeekSIFT(const char* featurefile, int& npoint, int& ndim)
{
  ...
  ///
  read(fd, &sfh, sizeof(sfh));
  if (sfh.szFeature != SIFT_NAME || !IsValidVersionName(sfh.szVersion) || sfh.npoint <= 0 || sfh.nLocDim <= 0 ||
      sfh.nDesDim != 128)
  {
    // incorrect version
    close(fd);
...
void FeatureData::SaveLocationFile(const char* szFeatureFile)
{
  ...
  float* p = &loc[0];
  int fid = open(szFeatureFile, O_CREAT | O_BINARY | O_WRONLY | O_TRUNC, S_IREAD | S_IWRITE);
  if (fid == -1)
    return;
...
void FeatureData::SortSIFT()
{
  ...
  if (nf == 0 || _locData == NULL || _desData == NULL)
    return;
...
{
  if (_locData)
    _locData->shrink(ndim, npoint < 0 ? _locData->npoint() : min(npoint, _locData->npoint()));
}
