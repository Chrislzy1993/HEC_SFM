...
void SetSolverOptions(const BundleAdjustmentOptions& options, ceres::Solver::Options* solver_options)
{
  ...
  solver_options->visibility_clustering_type = options.visibility_clustering_type;
  solver_options->logging_type = options.verbose ? ceres::PER_MINIMIZER_ITERATION : ceres::SILENT;
...
std::vector<int> GetIntrinsicsToOptimize(const OptimizeIntrinsicsType& intrinsics_to_optimize)
{
  ...
  if ((intrinsics_to_optimize & OptimizeIntrinsicsType::FOCAL_LENGTH) == OptimizeIntrinsicsType::NONE)
  {
    constant_intrinsics.emplace_back(Camera::FOCAL_LENGTH);
  }
  if ((intrinsics_to_optimize & OptimizeIntrinsicsType::ASPECT_RATIO) == OptimizeIntrinsicsType::NONE)
  {
    constant_intrinsics.emplace_back(Camera::ASPECT_RATIO);
  }
  if ((intrinsics_to_optimize & OptimizeIntrinsicsType::SKEW) == OptimizeIntrinsicsType::NONE)
  {
    constant_intrinsics.emplace_back(Camera::SKEW);
  }
  if ((intrinsics_to_optimize & OptimizeIntrinsicsType::PRINCIPAL_POINTS) == OptimizeIntrinsicsType::NONE)
  {
    constant_intrinsics.emplace_back(Camera::PRINCIPAL_POINT_X);
    constant_intrinsics.emplace_back(Camera::PRINCIPAL_POINT_Y);
  }
  if ((intrinsics_to_optimize & OptimizeIntrinsicsType::RADIAL_DISTORTION) == OptimizeIntrinsicsType::NONE)
  {
...
void AddCameraIntrinsicsToProblem(const std::vector<int>& constant_intrinsics, double* camera_intrinsics,
                                  ceres::Problem* problem)
{
  ...
  else if (constant_intrinsics.size() > 0)
  {
    ceres::SubsetParameterization* subset_parameterization =
        new ceres::SubsetParameterization(Camera::kIntrinsicsSize, constant_intrinsics);
    problem->AddParameterBlock(camera_intrinsics, Camera::kIntrinsicsSize, subset_parameterization);
  }
  else
  {
    problem->AddParameterBlock(camera_intrinsics, Camera::kIntrinsicsSize);
...
std::unordered_map<CameraIntrinsicsGroupId, double*> GetSharedIntrinsicsMap(const std::unordered_set<ViewId>& view_ids,
                                                                            Reconstruction* reconstruction)
{
  std::unordered_map<CameraIntrinsicsGroupId, double*> shared_intrinsics_by_group_id;
  ...
  for (const ViewId view_id : view_ids)
  {
    ...
    const CameraIntrinsicsGroupId intrinsics_group_id = reconstruction->CameraIntrinsicsGroupIdFromViewId(view_id);
    ...
    {
      shared_intrinsics_by_group_id[intrinsics_group_id] = view->MutableCamera()->mutable_intrinsics();
    }
  }
...
void CopySharedIntrinsicsToViews(
    const std::unordered_map<CameraIntrinsicsGroupId, double*>& shared_intrinsics_by_group_id,
    Reconstruction* reconstruction)
{
  for (const auto& shared_intrinsics : shared_intrinsics_by_group_id)
  {
    ...
    const auto& views_in_intrinsics_group = reconstruction->GetViewsInCameraIntrinsicGroup(shared_intrinsics.first);
    ...
    for (const ViewId view_id : views_in_intrinsics_group)
    {
      ...
      {
        std::copy(shared_intrinsics_for_group, shared_intrinsics_for_group + Camera::kIntrinsicsSize, intrinsics);
      }
    }
  }
}
...
BundleAdjustmentSummary BundleAdjustPartialHandEye(const BundleAdjustmentOptions& options,
                                                   ...
                                                   Reconstruction* reconstruction, Poses* handposes,
                                                   HandEyeTransformation* handeyetrans)
{
  ...
  const std::vector<int> constant_intrinsics = GetIntrinsicsToOptimize(options.intrinsics_to_optimize);
  ...
  for (auto& shared_intrinsics : shared_intrinsics_by_group_id)
  {
    ...
    AddCameraIntrinsicsToProblem(constant_intrinsics, shared_intrinsics.second, &problem);
  }
  ...
  for (const ViewId view_id : view_ids)
  {
    ...
    const CameraIntrinsicsGroupId intrinsics_group_id = reconstruction->CameraIntrinsicsGroupIdFromViewId(view_id);
    double* shared_intrinsics = FindOrDie(shared_intrinsics_by_group_id, intrinsics_group_id);
    ...
    for (const TrackId track_id : view->TrackIds())
    {
      ...
      problem.AddResidualBlock(HandEyeReprojectionError::Create(*feature, handposes->at(view_id)), loss_function.get(),
                               handeyetrans->Mutable_HandEyeParameter(), shared_intrinsics,
  ...
  for (const TrackId track_id : track_ids)
  {
    ...
    for (const ViewId view_id : observed_view_ids)
    {
      ...
      Camera* camera = view->MutableCamera();
      const CameraIntrinsicsGroupId intrinsics_group_id = reconstruction->CameraIntrinsicsGroupIdFromViewId(view_id);
      const bool variable_shared_intrinsics = ContainsKey(shared_intrinsics_by_group_id, intrinsics_group_id);
      ...
      {
        shared_intrinsics = FindOrDie(shared_intrinsics_by_group_id, intrinsics_group_id);
      }
      else
      {
  ...
  {
    solver_options.inner_iteration_ordering.reset(new ceres::ParameterBlockOrdering(*parameter_ordering));
  ...
  summary.setup_time_in_seconds = internal_setup_time + solver_summary.preprocessor_time_in_seconds;
...
BundleAdjustmentSummary BundleAdjusthandEye(const BundleAdjustmentOptions& options, Reconstruction* reconstruction,
                                            Poses* handposes, HandEyeTransformation* handeyetrans)
{
  const auto& view_ids = reconstruction->ViewIds();
  const auto& track_ids = reconstruction->TrackIds();
  const std::unordered_set<ViewId> view_ids_set(view_ids.begin(), view_ids.end());
  const std::unordered_set<TrackId> track_ids_set(track_ids.begin(), track_ids.end());
  return BundleAdjustPartialHandEye(options, view_ids_set, track_ids_set, reconstruction, handposes, handeyetrans);
}

// Bundle adjust a single view.
BundleAdjustmentSummary BundleAdjustView(const BundleAdjustmentOptions& options, const ViewId view_id,
                                         Reconstruction* reconstruction, Poses* handposes,
                                         HandEyeTransformation* handeyetrans)
{
  ...
  return BundleAdjustPartialHandEye(ba_options, view_ids, track_ids, reconstruction, handposes, handeyetrans);
}

// Bundle adjust a single track.
BundleAdjustmentSummary BundleAdjustTrack(const BundleAdjustmentOptions& options, const TrackId my_track_id,
                                          Reconstruction* reconstruction, Poses* handposes,
                                          HandEyeTransformation* handeyetrans)
{
  std::unordered_set<ViewId> view_ids;
