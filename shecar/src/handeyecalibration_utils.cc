...
void SetCameraPosesFromHandPoses(const Poses& handposes, HandEyeTransformation* handeyetrans,
                                 Reconstruction* reconstruction)
{
  ...
  for (int i = 0; i < handposes.size(); i++)
  {
    ...
    if (view == nullptr)
    {
      LOG(WARNING) << "Trying to set the pose of View " << i << " which does not exist in the reconstruction.";
      continue;
    }
    ...
    view->MutableCamera()->SetPosition(handeyeposition -
                                       handorientation * handeyerotation.transpose() * handeyetranslation);
...
{
  ceres::RotationMatrixToAngleAxis(ceres::ColumnMajorAdapter3x3(pose.topLeftCorner(3, 3).data()), ar);
