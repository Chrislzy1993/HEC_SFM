...
public:
  ...
  template <typename T>
  bool operator()(const T* handeyetrans, const T* camera_intrinsics, const T* point_parameters,
                  T* reprojection_error) const
  {
    // Do not evaluate invalid camera configurations.
    if (camera_intrinsics[Camera::FOCAL_LENGTH] < T(0.0) || camera_intrinsics[Camera::ASPECT_RATIO] < T(0.0))
    {
      return false;
    }
    ...
    ceres::AngleAxisToRotationMatrix(handeyetrans + HandEyeTransformation::ROTATION,
                                     ceres::ColumnMajorAdapter3x3(handeyerotation.data()));
    //
    Eigen::Matrix<T, 3, 1> handeyetranslation =
        Eigen::Map<const Eigen::Matrix<T, 3, 1>>(handeyetrans + HandEyeTransformation::TRANSLATION);
    ...
    Eigen::Matrix<T, 3, 1> cameraposition =
        handpose_t.topRightCorner(3, 1) - cameraorientation.transpose() * handeyetranslation;
    // map camera orientation to camera pose parameter
    ceres::RotationMatrixToAngleAxis((const T*)cameraorientation.data(), (T*)cameraposeparameter + Camera::ORIENTATION);
    ...
    T reprojection[2];
    ProjectPointToImage(cameraposeparameter, camera_intrinsics, point_parameters, reprojection);
