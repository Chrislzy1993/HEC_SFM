...
DEFINE_string(hand_poses_file, "", "hand poses file containing hand poses");
...
int main(int argc, char* argv[])
{
  ...
  CHECK(reconstruction_builder.BuildHandEyeCalibration(&handeyetrans)) << "Could not create a reconstruction.";
  ...
  const std::string output_file = theia::StringPrintf("%s", FLAGS_output_reconstruction.c_str());
  CHECK(theia::WriteReconstruction(*reconstructions, output_file)) << "Could not write reconstruction to file.";

  Eigen::IOFormat fmt;
  fmt.precision = Eigen::FullPrecision;
  cout << "Estimated hand-eye transform:" << handeyetrans.GetHandEyeRotationAsRotationMatrix().format(fmt) << std::endl
       << handeyetrans.GetHandEyeTranslation().format(fmt) << endl;
}
