...
inline DescriptorExtractorType StringToDescriptorExtractorType(const std::string& descriptor)
{
  if (descriptor == "SIFT")
  {
...
inline FeatureDensity StringToFeatureDensity(const std::string& feature_density)
{
...
inline MatchingStrategy StringToMatchingStrategyType(const std::string& matching_strategy)
{
  ...
  {
    LOG(FATAL) << "Invalid matching strategy specified. Using BRUTE_FORCE instead.";
    return MatchingStrategy::BRUTE_FORCE;
  }
}

inline ReconstructionEstimatorType StringToReconstructionEstimatorType(const std::string& reconstruction_estimator)
{
  ...
  {
    LOG(FATAL) << "Invalid reconstruction estimator type. Using GLOBAL instead.";
    return ReconstructionEstimatorType::GLOBAL;
  }
}

inline GlobalRotationEstimatorType StringToRotationEstimatorType(const std::string& rotation_estimator)
{
...
inline GlobalPositionEstimatorType StringToPositionEstimatorType(const std::string& position_estimator)
{
...
inline OptimizeIntrinsicsType StringToOptimizeIntrinsicsType(const std::string& intrinsics_to_optimize)
{
  CHECK_GT(intrinsics_to_optimize.size(), 0) << "You must specify which camera intrinsics parametrs to optimize. "
  ...
  CHECK_GT(intrinsics.size(), 0) << "Could not decipher any valid camera intrinsics.";
  ...
  for (int i = 0; i < intrinsics.size(); i++)
  {
    ...
    {
      LOG(FATAL) << "Invalid option for intrinsics_to_optimize: " << intrinsics[i];
    }
  }
  return intrinsic_params;
}

inline LossFunctionType StringToLossFunction(const std::string& loss_function_type)
{
