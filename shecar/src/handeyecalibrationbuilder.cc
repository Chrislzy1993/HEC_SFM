...
{
  CHECK(reconstruction_->NumViews() == poses->size()) << "numer of views " << reconstruction_->NumViews()
                                                      << " is unequal to that of hand poses";
...
bool HandEyeCalibrationBuilder::BuildHandEyeCalibration(HandEyeTransformation* handeyetrans)
{
  ...
  LOG(INFO) << "Attempting to reconstruct " << reconstruction_->NumViews() << " images from " << view_graph_->NumEdges()
            << " two view matches.";
  ...
  const auto& summary =
      handeyecalibrationestimator->Estimate(view_graph_.get(), reconstruction_.get(), &hand_poses_, handeyetrans);
