...
HandEyeCalibrationEstimator::HandEyeCalibrationEstimator(const ReconstructionEstimatorOptions& options)
...
ReconstructionEstimatorSummary HandEyeCalibrationEstimator::Estimate(ViewGraph* view_graph,
                                                                     ...
                                                                     HandEyeTransformation* handeyetrans)
{
  ...
  for (int i = 0; i < options_.num_retriangulation_iterations + 1; i++)
  {
    ...
    if ((Eigen::Map<Eigen::Matrix<double, 3, 1>>(old_handeyetrans + HandEyeTransformation::TRANSLATION) -
         Eigen::Map<Eigen::Matrix<double, 3, 1>>(handeyetrans->Mutable_HandEyeParameter() +
                                                 HandEyeTransformation::TRANSLATION))
            .norm() < 1e-6)
      break;
    else
      std::memcpy(old_handeyetrans, handeyetrans->HandEyeParameter(), 6 * sizeof(double));
...
void HandEyeCalibrationEstimator::EstimateStructure(Poses* handposes, HandEyeTransformation* handeyetrans)
{
  // Estimate all tracks.
