...
{
public:
  HandEyeTrackEstimator(const Options& options, Reconstruction* reconstruction, Poses* handpose,
                        HandEyeTransformation* handeyetrans)
    : TrackEstimator(options, reconstruction), handeyetrans_(handeyetrans), handpose_(handpose)
  {
  }
  // Attempts to estimate all unestimated tracks.
  TrackEstimator::Summary HandEyeEstimateAllTracks();
  TrackEstimator::Summary HandEyeEstimateTracks(const std::unordered_set<TrackId>& track_ids);
